...
const ReviewList = () => {
    return (
        <div className='flex flex-col gap-2'>
            <h1 className='text-2xl font-medium mb-3'>Existing Reviews</h1>
            {reviews.map((rev, i) => (
                <div className='flex gap-4 h-40 border overflow-hidden rounded-lg shadow-md' key={i}>
                    ...
                    <div className='h-full flex flex-col justify-center gap-2 p-2  w-full'>
                        ...
                        <p className='text-lg text-gray-400'>{rev.date}</p>
                        <Ratings rating={rev.rating} />
                        <p className='text-lg text-gray-400 truncate max-w-80' title={rev.review}>{rev.review}</p>
                    </div>
                </div>
            ))}
        </div>
